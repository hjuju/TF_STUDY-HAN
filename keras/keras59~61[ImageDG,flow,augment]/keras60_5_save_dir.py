...
둘 다 이터레이터(리스트, 순서, .next로 하나하나 빼줌)로 출력
프린트할때도 x_augumented를 호출하게 됨(이터레이터이기 때문임) -> 배치사이즈만큼 save_to_dir도 실행이 됨

## .next()를 해줄때와 안해줄때의 차이 알아보기 (X)
...
augment_size = 10 # 이미지 증폭하는 개수 선택
...
x_augmented = train_datagen.flow(x_augmented, np.zeros(augment_size),
                                     ...
                                     save_to_dir='d:/temp/').next()[0] # x만 출력 , 저장할 위치만 설정

# 밑에 ic를 하면 .next에의해 리터레이터가 실행됨
...
# ic(x_augmented[0][1][:10]) # y값 10개 출력
