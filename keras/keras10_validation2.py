...
x_train = np.array(x[:7]) # 실질적으로 훈련, 공부하는 것 / 7개의 데이터를 
y_train = np.array(y[:7])
x_test = np.array(x[7:10]) # 테스트 데이터는 오직 평가를 위해서 사용
y_test = np.array(y[7:10])
x_val = np.array(x[10:13]) # 머신이 공부를 하고, 문제집을 푸는 것(model.fit에 명시)
y_val = np.array(y[10:13]) # 훈련을 할 때 검증, 반복횟수에 따라 검증 한 뒤 다시 훈련, 다시 검증 작업을 하게 되면 성능향상
