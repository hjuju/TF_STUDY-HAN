...
parameter = [{"C":[1, 10, 100, 1000], "kernel":['linear'], 'degree':[3,4,5]}, # 4 * 5(kfold숫자만큼) -> 20번 돌아감
             {"C":[1, 10, 100], "kernel":["rbf"], "gamma":[0.001, 0.0001]}, # 3 * 1 * 3 * 5 -> 30번
             {"C":[1, 10, 100, 1000], "kernel":["sigmoid"], "gamma":[0.001,0.001, 0.0001], 'degree':[3,4]}] # 4 * 1 * 2 * 5 = 40번 => 총 90번 모델이 돌아감


model = RandomizedSearchCV(SVC(), parameter, cv=kfold, verbose=1) # 사용할 모델, parameter(정의), cv 명시 /  텐서플로로 하면 모델에 텐서플로 모델이 들어가면 됨
