...
model = RandomizedSearchCV(RandomForestClassifier(), parameter, cv=kfold) # 사용할 모델, parameter(정의), cv 명시 /  텐서플로로 하면 모델에 텐서플로 모델이 들어가면 됨
# model = SVC(C=1, kernel='linear')

model.fit(x_train,y_train)

print('최적의 매개변수: ', model.best_estimator_) # cv를 통해 나온 값 / GridSearchCV를 통해서만 출력 가능
print('best_prams: ', model.best_params_)
...
최적의 매개변수:  RandomForestClassifier(max_depth=6, min_samples_leaf=8, min_samples_split=8,
                       n_jobs=-1)
best_score:  0.956043956043956

최적의 매개변수:  RandomForestClassifier(max_depth=8, min_samples_leaf=8, min_samples_split=8,
