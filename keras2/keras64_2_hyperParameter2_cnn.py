...
#2 모델
def build_model(layers=Conv2D, drop=0.5, optimizer='adam'):
    inputs = Input(shape=(28,28,1), name='input')
    x = layers(512, kernel_size = (2,2), activation='relu', name='hidden1')(inputs)
    x = Dropout(drop)(x)
    x = layers(256, kernel_size = (2,2),activation='relu', name='hidden2')(x)
    x = Dropout(drop)(x)
    x = layers(128, kernel_size = (2,2), activation='relu', name='hidden3')(x)
    x = Dropout(drop)(x)
    x = Flatten()(x)
