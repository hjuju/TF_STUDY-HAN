...
model.compile(loss='binary_crossentropy', optimizer=optimizer, metrics=['accuracy']) # mse는 선을 그은 것에서 선과 비교함, 이진 분류에는 binary_crossentropy
es = EarlyStopping(monitor='loss', patience=10, mode='auto', verbose=1)
hist = model.fit(x_train, y_train, epochs=40, verbose=1, batch_size=8, validation_split=0.1, shuffle=True, callbacks=[es, reduce_lr])
...
'''
ReduceLR
ic| 'loss:', loss[0]: 0.12690086662769318
ic| 'accuracy', loss[1]: 0.9561403393745422
이진분류
train size = 0.7
model.add(Dense(256, activation='relu', input_shape =(30,)))
model.add(Dense(128, activation='relu'))
